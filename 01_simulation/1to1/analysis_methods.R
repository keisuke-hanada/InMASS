...
ipd_method <- function(formula, dat) {
  nsim <- dat$params$nsim
  cl <- makeCluster(parallel::detectCores()-1)
  registerDoParallel(cl)
  
  res_list <- foreach(i = 1:nsim) %dopar% {
    d <- subset(dat$target_ipd, nsim == i)
    lm(formula, data = d)
  }
  
  stopCluster(cl)
  
  res_list$params <- dat$params
  return(res_list)
}
...
meta_method <- function(formula, dat, subfname) {
  ...
  res_list <- foreach(i = 1:nsim, 
                      ...
                      .export = c("iwlm", "inmass"), .packages = c("stats", "metafor")) %dopar% {
    ...
    fit <- rma.uni(yi=data.ma.mean$yik, vi=v2y, mods=formula.ma, data=data.ma.mean)
    res <-   list(
...
propose_method <- function(formula, dat, alpha = 0.05){
  formula.ma <- as.formula(dat$params$formula)
  nsim <- dat$params$nsim
  strata <- dat$params$strata
  
  cl <- makeCluster(parallel::detectCores()-8)
  registerDoParallel(cl)
  
  res_list <- foreach(i = 1:nsim, .export = c("iwlm", "inmass"), .packages = c("stats", "metafor")) %dopar% {
    data.ipd <- subset(dat$target_ipd, subset=nsim==i)
    data.ma.mean <- subset(dat$strata_ad, subset=nsim==i & var=="mean")
    data.ma.var <- subset(dat$strata_ad, subset=nsim==i & var=="var")
    
    res <- inmass(formula=formula, formula.ma=formula.ma,
           data.ma.mean=data.ma.mean, data.ma.var=data.ma.var, 
           data.ipd=data.ipd, strata=strata)
    gc()
    return(res)
  }
  stopCluster(cl)
...
inmass <- function(formula, formula.ma, data.ma.mean, data.ma.var, data.ipd, strata,
  ...
  # compdat <- rbind(
  #   data.frame(d="IPD", x2k=dat01$strata_ipd$x2k[dat01$strata_ipd$nsim==i], yik=dat01$strata_ipd$yik[dat01$strata_ipd$nsim==i]),
  #   data.frame(d="rIPD", x2k=dat01ripd$x2k, yik=dat01ripd$yik)
  # )
  # print(
  # ggplot(data=compdat, aes(x=x2k, y=yik, color=d)) +
  #   geom_point()
  # )
  ...
  # print(
  #   boxplot(weight ~ strata, dat01all)
  # )
  if(ps_meta == 2){
  ...
  # c(n1, n1h, n0, n0h)
  # ggplot(data=dat01all, aes(x=x2k, y=yik, color=x1k)) +
  # geom_point()
  # sum((dat01all$weight*dat01all$yik)[dat01all$x1k==1])/sum(dat01all$weight[dat01all$x1k==1]) -
  #   sum((dat01all$weight*dat01all$yik)[dat01all$x1k==0])/sum(dat01all$weight[dat01all$x1k==0])
...
# ### example of inmass ###
# data.ipd <- subset(dat$target_ipd, subset=nsim==1)
# data.ma.mean <- subset(dat$strata_ad, subset=nsim==1 & var=="mean")
# data.ma.var <- subset(dat$strata_ad, subset=nsim==1 & var=="var")
# formula <- yik ~ x1k
# formula.ma <- as.formula(dat$params$formula)
# strata <- 10
# 
# test1 <- imiss(formula, formula.ma, data.ma.mean, data.ma.var, data.ipd, strata)
# print(test1)
# summary(test1)
# confint(test1)
# 
...
### total of integrate_meta_into_specific_study for only control ###
inmass_control <- function(formula, formula.ma, data.ma.mean, data.ma.var, data.ipd, strata,
  ...
  psval <- predict(ps_model, type="response")[ps_dat$id==0]
  dat01all$weight <- psval / (1 - psval) * (n_source/n_target)
  # dat01all$weight[dat01all[[vars[2]]]==0] <- psval / (1 - psval) * (n_source/n_target)
  # dat01all$weight[dat01all[[vars[2]]]==1] <- 1
  # print(
  #   boxplot(weight ~ strata, dat01all)
  # )
  ...
  n0h <- sum(dat01all$weight[dat01all[[vars[2]]]==0])
  # c(n1, n1h, n0, n0h)
  # ggplot(data=dat01all, aes(x=x2k, y=yik, color=x1k, group=x1k)) +
  #   geom_point()
  # sum((dat01all$weight*dat01all$yik)[dat01all$x1k==1])/sum(dat01all$weight[dat01all$x1k==1]) -
  #   sum((dat01all$weight*dat01all$yik)[dat01all$x1k==0])/sum(dat01all$weight[dat01all$x1k==0])

...
generate_main_replicate <- function(replicate, spec, n_external, mu_external, seed) {
  set.seed(seed)
  K <- as.integer(spec$K)
  n <- as.integer(spec$n)
  sigma <- spec$sigma
  ...
  list(target_ipd = target_ipd, strata_ipd = external_ipd, strata_ad = aggregate_data)
}

generate_main_data <- function(spec, seed, n_workers = 1L, cluster = NULL) {
  set.seed(seed)
  K <- as.integer(spec$K)
  n <- as.integer(spec$n)
  ...
  replicate_jobs <- parallel_lapply(seq_len(nsim), function(replicate) {
    generate_main_replicate(
      replicate, spec, n_external, mu_external,
      replicate_seed(spec$scenario_id, replicate, "data", seed)
    )
  }, n_workers = n_workers, cluster = cluster)
  ...
  list(
    target_ipd = do.call(rbind, lapply(replicate_jobs, `[[`, "target_ipd")),
    strata_ipd = do.call(rbind, lapply(replicate_jobs, `[[`, "strata_ipd")),
    strata_ad = do.call(rbind, lapply(replicate_jobs, `[[`, "strata_ad")),
    params = params
  )
}

generate_multicov_replicate <- function(replicate, spec, n_external, mu1_external, mu2_external, seed) {
  set.seed(seed)
  K <- as.integer(spec$K)
  n <- as.integer(spec$n)
  sigma <- spec$sigma
...
generate_multicov_data <- function(spec, seed, n_workers = 1L, cluster = NULL) {
  set.seed(seed)
  K <- as.integer(spec$K)
  n <- as.integer(spec$n)
  nsim <- as.integer(spec$nsim)
  ...
  replicate_jobs <- parallel_lapply(seq_len(nsim), function(replicate) {
    generate_multicov_replicate(
      replicate, spec, n_external, mu1_external, mu2_external,
      replicate_seed(spec$scenario_id, replicate, "data", seed)
    )
  }, n_workers = n_workers, cluster = cluster)
...
generate_nonlinear_replicate <- function(replicate, spec, n_external, mu_external, seed) {
  set.seed(seed)
  K <- as.integer(spec$K)
  n <- as.integer(spec$n)
  sigma <- spec$sigma
...
generate_nonlinear_data <- function(spec, seed, n_workers = 1L, cluster = NULL) {
  set.seed(seed)
  K <- as.integer(spec$K)
  n <- as.integer(spec$n)
  nsim <- as.integer(spec$nsim)

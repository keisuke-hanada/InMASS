...
ipd_method <- function(formula, dat) {
  ...
  res_list <- foreach(i = 1:nsim, .packages = c("stats")) %dopar% {
...
propose_method <- function(formula, dat, alpha = 0.05){
  ...
  res_list <- foreach(i = 1:nsim, .export = c("iwlm", "inmass_control"), .packages = c("stats", "metafor")) %dopar% {
    ...
    inmass_control(formula=formula, formula.ma=formula.ma,
